...
exports.addVideo = async (req, res) => {
  try {
    ...
    const result = await addDemoVideo(
      req.params.courseId,
      req.params.curriculumIndex,
      req.params.partIndex,
      video_url,
    );
    if (!result.success) {
      return res
...
const addDemoVideo = async (
  ...
  try {
    ...
    if (
      !course.curriculum[curriculumIndex] ||
      !course.curriculum[curriculumIndex].parts[partIndex]
    ) {
      return {
        success: false,
        message: 'Part not found in curriculum',
      };
    }
    course.curriculum[curriculumIndex].parts[
      partIndex
    ].demoVideo = video_url;
    await course.save();
    return {
...
exports.addTestimonial = async (req, res) => {
  try {
    ...
    // Push the testimonial to the array
    course.testimonials.push({ testimonial });
    await course.save();

    res.status(200).json({
      success: true,
      message: 'Testimonial added successfully.',
      testimonials: course.testimonials, // optionally return them
    });
  } catch (error) {
    console.error('Error adding testimonial:', error);
    res.status(500).json({
      success: false,
      message: 'Server error. Failed to add testimonial.',
    });
  }
};
exports.deleteTestimonial = async (req, res) => {
  try {
    ...
    const testimonialIndex = course.testimonials.findIndex(
      (testi) => testi._id.toString() === testimonialId,
    );
    ...
    course.testimonials.splice(testimonialIndex, 1);
    ...
    res.status(500).json({
      success: false,
      message:
        'Server error. Failed to delete testimonial.',
    });
  }
};
